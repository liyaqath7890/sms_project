...
export function NotificationCenter() {
  ...
  const { addNotification, withLoading } = useApp();
  ...
  const loadNotifications = async () => {
    try {
      const data = await dataManager.getNotifications();
  ...
  const markAsRead = async (id) => {
    try {
      await dataManager.markNotificationAsRead(id);
  ...
  const markAllAsRead = async () => {
    ...
    try {
      await withLoading('mark-all-read', async () => {
        // This would call the API to mark all as read
        const unreadIds = notifications.filter(n => !n.read).map(n => n.id);
        await Promise.all(unreadIds.map(id => dataManager.markNotificationAsRead(id)));
      });
