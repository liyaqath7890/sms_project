...
const PageLoader = () => (
  <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center">
    <div className="text-center">
      ...
      <p className="text-white/70">Loading SMS...</p>
    </div>
);
...
// Fix StudentProfile
const sp = `import React, { useState, useEffect } from 'react';
...
`;
fs.writeFileSync('src/features/student/pages/StudentProfile.jsx', sp);

// Fix AddTeacher
const at = `import React, { useState } from 'react';
  ...
  return (
    <div style={{padding:'2rem',maxWidth:800,margin:'0 auto'}}>
      ...
      <form onSubmit={submit} style={{display:'grid',gap:'1rem',marginTop:'1.5rem'}}>
        <input name="firstName" placeholder="First Name" onChange={handle} required style={inp} />
        <input name="lastName" placeholder="Last Name" onChange={handle} required style={inp} />
        <input name="email" type="email" placeholder="Email" onChange={handle} style={inp} />
        <input name="phone" placeholder="Phone" onChange={handle} style={inp} />
        <input name="subject" placeholder="Subject" onChange={handle} required style={inp} />
        <input name="qualification" placeholder="Qualification" onChange={handle} style={inp} />
        <input name="experienceYears" type="number" placeholder="Experience (Years)" onChange={handle} style={inp} />
        <button type="submit" style={{...inp,background:'#3b82f6',color:'white',cursor:'pointer',fontWeight:600}}>Save Teacher</button>
      </form>
    </div>
  );
}
const inp = {width:'100%',padding:'0.75rem',border:'1px solid #e5e7eb',borderRadius:8,fontSize:15,outline:'none'};
`;
fs.writeFileSync('src/features/teacher/pages/AddTeacher.jsx', at);

// Fix Timetable
const tt = `import React, { useState } from 'react';
...
`;
fs.writeFileSync('src/features/schedule/pages/Timetable.jsx', tt);

// Fix ReportCard
const rc = `import React, { useState, useEffect } from 'react';
...
`;
fs.writeFileSync('src/features/gradebook/pages/ReportCard.jsx', rc);

// Fix GlobalAnalytics
const ga = `import React from 'react';
...
`;
fs.writeFileSync('src/features/reports/pages/GlobalAnalytics.jsx', ga);
